...
def get_urls(driver, url_set) -> set:
    ...
    url_raw = soup.find_all("a", class_="sc-80282803-7 kDifwX")
    for x in url_raw:
...
url_list = set()
MAX_WAIT_TIME = 200 # make sure this is big enough so it wont stop unexpectedly because of latency reasons
...
for url in url_list_list:
    cursor.execute("SELECT 1 FROM buildings WHERE website_id = %s", (url.removeprefix("https://bina.az/items/"),)) # comma is because it needs to be a tuple for some reason
    if cursor.fetchone():
